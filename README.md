...
**asc-mcp** is a Swift-based MCP server that bridges [Claude](https://claude.ai) (or any MCP-compatible host) with the [App Store Connect API](https://developer.apple.com/documentation/appstoreconnectapi). It exposes **399 tools** across 33 worker domains, enabling you to automate your entire iOS/macOS release workflow through natural language.
...
> **Note:** Windsurf has a 100-tool limit. The server exposes 399 tools by default, so you must use `--workers` to select a subset. See [Worker Filtering](#worker-filtering) below.
...
The server exposes **399 tools** across 33 worker domains. Some MCP clients impose a tool limit (e.g., Windsurf caps at 100). Use `--workers` to enable only the workers you need:
...
|---|---:|---:|
| All workers (default) | 399 | **~44,000** |
...
**399 tools** organized across 33 worker domains (use `--workers` to filter — see [Worker Filtering](#worker-filtering)):
...
└── Workers/                        # MCP tool implementations (33 worker domains + MainWorker router)
